...
const navItems = [
  ...
  { to: '/delivery-challans', label: 'Delivery Challans', icon: FileCheck },
  { to: '/shipments', label: 'Shipments', icon: Truck },
  { to: '/returns', label: 'Returns', icon: RotateCcw },
  { to: '/replacements', label: 'Replacements', icon: ArrowRightLeft },
  { to: '/advance-replacements', label: 'Adv. Replacements', icon: RefreshCw },
...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground overflow-hidden selection:bg-rentr-primary/30 selection:text-white">
      ...
      {/* Desktop Sidebar - Extended with labels */}
      <aside className="hidden lg:flex w-64 h-screen border-r border-foreground/[0.08] bg-background/80 backdrop-blur-xl flex-col sticky top-0 z-50">
        ...
        <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const isExactDashboard = item.to === '/' && location.pathname === '/';
            const active = item.to === '/' ? isExactDashboard : isActive;
        ...
        <div className="border-t border-foreground/[0.06] p-4 space-y-3">
          ...
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <button className="p-1.5 rounded-lg text-foreground/20 hover:text-foreground/50 transition-colors">
        ...
        <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
          {navItems.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex items-center gap-3 px-3 py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-[0.15em] transition-all duration-300 border',
                  isActive
                    ? 'bg-rentr-primary/8 text-rentr-primary border-rentr-primary/15'
                    : 'text-foreground/25 hover:text-foreground/60 hover:bg-foreground/[0.02] border-transparent'
                )
              }
            >
              <Icon size={18} />
