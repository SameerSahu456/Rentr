...
export function RentrIcon({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  const sizeClasses = {
    sm: "text-xl",
    md: "text-3xl",
    lg: "text-4xl",
  ...
  return (
    <div className={cn("flex flex-col items-end select-none cursor-default group", className)}>
      <span className={cn("font-brand font-black text-rentr-primary transition-transform duration-500 group-hover:scale-110", sizeClasses[size])}>R</span>
